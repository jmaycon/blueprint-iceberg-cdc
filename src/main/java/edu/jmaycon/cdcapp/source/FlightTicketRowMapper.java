...
public class FlightTicketRowMapper {

    public FlightTicketAvro map(Row row) {
        ...
        FlightTicketAvro.Builder builder = FlightTicketAvro.newBuilder()
                .setTicketUuid(UUID.fromString(row.getString(row.fieldIndex("ticket_uuid"))))
                .setTicketId(row.getString(row.fieldIndex("ticket_id")))
